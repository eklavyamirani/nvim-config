...
vim.defer_fn(function()
  ...
  local ok, err = xpcall(function()
    ...
    local main = lib.get_current_view().cur_layout:get_main_win()
    vim.api.nvim_set_current_win(main.id)
    assert(vim.fn.maparg(' aa', 'n', false, true).desc == 'Review: ask about the current line', 'question shortcut missing in diff')
    assert(vim.fn.maparg(' aa', 'x', false, true).desc == 'Review: ask about selected code', 'visual question shortcut missing in diff')
    vim.fn.maparg('g?', 'n', false, true).callback()
    local help = table.concat(vim.api.nvim_buf_get_lines(0, 0, -1, false), '\n')
    assert(help:find('Review: ask about the current line', 1, true), 'question shortcut missing from native help')
    vim.cmd.close()
